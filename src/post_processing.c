...
post_processing_init(Arena* arena, Renderer* renderer)
{
    ...
    post_processing_state->camera_shake_cap        = 0.8;
...
post_processing_update(EngineTime time)
{
    ...
    g_post_processing_state->current_aberration_duration -= time.dt * 2;
    g_post_processing_state->current_aberration_duration = max(g_post_processing_state->current_aberration_duration, 0);
    g_post_processing_state->current_aberration_strength = powf(g_post_processing_state->current_aberration_duration, 1.2);
    ...
    g_post_processing_state->current_camera_shake_strength = powf(g_post_processing_state->current_camera_shake_duration, 1);
}

internal void
post_processing_move_camera(Vec2 position, EngineTime time)
{
    float32 x_shake = (sin(time.current_frame_time / 20.0f) * 0.4f) * g_post_processing_state->current_camera_shake_strength;
    float32 y_shake = (cos(time.current_frame_time / 10.0f) * 0.3f) * g_post_processing_state->current_camera_shake_strength;
    position.x += x_shake;
    position.y += y_shake;
